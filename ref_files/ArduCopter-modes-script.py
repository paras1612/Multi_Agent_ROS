...
def setGuidedMode():
    rospy.wait_for_service('/vehicle1/mavros/set_mode')
    try:
        flightModeService = rospy.ServiceProxy('/vehicle1/mavros/set_mode', mavros_msgs.srv.SetMode)
...
def setLandMode():
    rospy.wait_for_service('/vehicle1/mavros/cmd/land')
    try:
        landService = rospy.ServiceProxy('/vehicle1/mavros/cmd/land', mavros_msgs.srv.CommandTOL)
        #http://wiki.ros.org/mavros/CustomModes for custom modes
...
def setArm():
    rospy.wait_for_service('vehicle2/mavros/cmd/arming')
    try:
        armService = rospy.ServiceProxy('vehicle2/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
        armService(True)
    except rospy.ServiceException, e:
        print "Service arm call failed: %s"%e
        
def setDisarm():
    rospy.wait_for_service('/vehicle1/mavros/cmd/arming')
    try:
        armService = rospy.ServiceProxy('/vehicle1/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
        armService(False)
    except rospy.ServiceException, e:
        print "Service arm call failed: %s"%e
...
def setTakeoffMode():
    rospy.wait_for_service('/vehicle1/mavros/cmd/takeoff')
    try:
        takeoffService = rospy.ServiceProxy('/vehicle1/mavros/cmd/takeoff', mavros_msgs.srv.CommandTOL) 
